...
def train(
    ...
    while batch_data is not None:
        ...
        if batch_index % train_config.train_print_frequency == 0:
            ...
            print(f"Epoch: [{epoch + 1}][{batch_index}/{batches}]\t Loss: {losses.val:.4f} ({losses.avg:.4f})\t Acc@1: {acc1.val:.3f} ({acc1.avg:.3f})\t Acc@5: {acc5.val:.3f} ({acc5.avg:.3f})")
